...
`papr` is a token-efficient, command-line companion to the desktop app, built for
**autonomous agents** to drive over shell execution. It reads the same local
SQLite database the app maintains — through the shared `papr-core` crate, so its
queries, migrations and feed parsing can never drift from the desktop — letting
an agent catch you up on your feeds, search your subscriptions, triage unread
items, and pull new posts, all without a GUI.
